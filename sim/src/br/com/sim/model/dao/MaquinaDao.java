...
public class MaquinaDao {
	...
	public Maquina carregarPorNumeroDeSerie(String numeroDeSerie) {
		TypedQuery<Maquina> query = em
				.createQuery("select m from Maquina m where lower(m.numeroDeSerie) = :pNumeroDeSerie", Maquina.class);
	...
	public List<Maquina> filtrarMaquinas(String modelo, String descricao, Linha linha, Posto posto) {
		...
		sql.append("select * from maquinas as m ");
		sql.append("inner join postos as p on p.post_id = m.maqu_post_id ");
		sql.append("where true ");
		if (modelo != null && modelo.trim().length() > 0) {
			sql.append("and lower(m.maqu_modelo) like '" + modelo.trim().toLowerCase() + "%' ");
		}
		if (descricao != null && descricao.trim().length() > 0) {
			sql.append("and lower(m.maqu_descricao) like '" + descricao.trim().toLowerCase() + "%' ");
		}
		if (linha != null) {
			sql.append("and p.post_linh_id = " + linha.getId() + " ");
		}
		if (posto != null) {
			sql.append("and m.maqu_post_id = " + posto.getId() + " ");
		}
		sql.append("order by m.maqu_id ");
